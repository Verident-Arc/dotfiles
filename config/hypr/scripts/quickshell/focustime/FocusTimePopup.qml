...
Item {
   ...
   // -------------------------------------------------------------------------
    ...
    readonly property bool isTodaySelected: getIsoDate(window.activeDate) === getIsoDate(new Date())
    ...
    Process {
        id: liveFileReader
        command: ["/usr/bin/cat", window.stateFilePath]
    ...
    function syncWeekAppsModel() {
        for (let i = 0; i < window.weekAppsData.length; i++) {
            ...
            if (i < weekAppListModel.count) {
                ...
                weekAppListModel.setProperty(i, "appClass", aClass);
