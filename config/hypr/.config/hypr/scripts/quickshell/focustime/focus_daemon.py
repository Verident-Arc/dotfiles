...
def main():
    ...
    while True:
        ...
        # Periodic lock check in case IPC event was missed
        if is_locked():
            current_app_class, current_app_title = "Locked", "Locked"
        ...
        # Update JSON state. Heavy queries every 5s, total/current every 1s.
        current_time = time.time()
        if current_time - last_heavy_dump > 5:
            dump_state_to_json(c)
            last_heavy_dump = current_time
        else:
            # Fast update for total seconds and current app only
            try:
                today = date.today().isoformat()
                c.execute('SELECT SUM(seconds) FROM focus_log WHERE log_date = ?', (today,))
                ...
                if os.path.exists(STATE_FILE):
                    with open(STATE_FILE, 'r') as f:
                        data = json.load(f)
                    data["total"] = total
                    data["current"] = current_app_title
